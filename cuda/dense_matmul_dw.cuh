...
struct DenseMatMul_dw {
	static constexpr f64 SPARSE_SCALE = 1.0;

	// Output tile shape: dW has rows D_OUT and cols D_IN.
	static constexpr usize M = D_IN;
	static constexpr usize N = D_OUT;
	...
	static_assert(N % N_PER_BLOCK == 0);
	static_assert(K_STEP % 16 == 0);
	static_assert((M * sizeof(bf16)) % 16 == 0, "input rows must be 16B aligned");
	static_assert((N * sizeof(bf16)) % 16 == 0, "dO rows must be 16B aligned");
	static_assert(N_TILES % 2 == 0, "N_TILES must be even");

	static constexpr double flops(size_t seq_len) {
		return 2.0 * double(M) * double(N) * double(seq_len);
	}

	static X17_DEVICE void cp_async_ab(
		GMatrixDynSize<bf16, M_PER_BLOCK> gA_block,
		GMatrixDynSize<bf16, N_PER_BLOCK> gB_block,
		SMatrix<bf16, K_STEP * GMEM_PRELOAD, M_PER_BLOCK> sA_preload,
		SMatrix<bf16, K_STEP * GMEM_PRELOAD, N_PER_BLOCK> sB_preload,
		usize p,
		usize k_end
	) {
		if (p < k_end) {
			SMatrix<bf16, K_STEP, M_PER_BLOCK> sA_tile = tile_m<K_STEP>(sA_preload, p % GMEM_PRELOAD);
			SMatrix<bf16, K_STEP, N_PER_BLOCK> sB_tile = tile_m<K_STEP>(sB_preload, p % GMEM_PRELOAD);
			...
			cp_async_gmem_to_smem<THREADS_PER_BLOCK, K_STEP, N_PER_BLOCK>(
				threadIdx.x,
				tile_m<K_STEP>(gB_block, p),
				sB_tile,
				0, 0, 0, 0
			);
		}
	...
	X17_DEVICE void run(
		...
		usize k_iters = seq_len / K_STEP;
		GMatrixDynSize<bf16, M> gA{A, seq_len};
		GMatrixDynSize<bf16, N> gB{B, seq_len};
		GMatrixDynSize<bf16, M_PER_BLOCK> gA_block = gA.template slice_n<M_PER_BLOCK>(blockIdx.x * M_PER_BLOCK);
		GMatrixDynSize<bf16, N_PER_BLOCK> gB_block = gB.template slice_n<N_PER_BLOCK>(blockIdx.y * N_PER_BLOCK);

		u32 smem = 0;
		SMatrix<bf16, K_STEP * GMEM_PRELOAD, M_PER_BLOCK> sA_preload{smem};
		SMatrix<bf16, K_STEP * GMEM_PRELOAD, N_PER_BLOCK> sB_preload{sA_preload._ptr + sA_preload.bytes()};
		...
		SMatrix<bf16, K_STEP, N_PER_BLOCK> sB = tile_m<K_STEP>(sB_preload, 0);
		...
		X17_UNROLL for (usize k_tile = 0; k_tile < K_TILES; ++k_tile) {
			X17_UNROLL for (usize ni = 0; ni < N_TILES; ++ni) {
				smem_tile_to_fragment_trans(sB, k_tile * 16, warp_n + ni * 16, rB[k_tile][ni]);
			}
		...
		X17_UNROLL for (usize k_step = 0; k_step < k_iters; ++k_step) {
			{ // Get more data from GMEM
				...
				sB = tile_m<K_STEP>(sB_preload, (k_step + 1) % GMEM_PRELOAD);
