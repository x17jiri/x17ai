...
pub struct AttnKernelArgs<'a> {
	/// Shape: `[seq_len, HEAD_DIM * N_HEADS]`.
	pub q: &'a Tensor,

	/// Shape: `[seq_len, 2 * HEAD_DIM * N_HEADS]`.
	pub kv: &'a Tensor,
...
impl AttnKernelLauncher {
	fn new(
		...
		let metadata = kernel_build::read_metadata_json::<AttnLauncherMetadata>(
			metadata_path, diag
		)?;
		if metadata.THREADS_PER_BLOCK == 0
			|| metadata.SMEM_BYTES == 0
			|| !metadata.Q_PER_BLOCK.is_power_of_two() // Note: this also ensures != 0
		{
			cold_path();
	...
	fn launch(&self, args: AttnKernelArgs) -> Result<(), ErrPack<TensorOpError>> {
		...
		let [seq_len, _q_cols] = tensor_check::matrix_shape(
			"attention", "q", q, [None, Some(config.q_stride)],
			&self.device, config.dtype
		)?;
		tensor_check::matrix_shape(
			"attention", "kv", kv, [Some(seq_len), Some(config.kv_stride)],
			&self.device, config.dtype
		)?;
		tensor_check::matrix_shape(
			"attention", "sink_k", sink_k, [Some(config.n_heads), Some(config.head_dim)],
			&self.device, config.dtype
		)?;
		tensor_check::matrix_shape(
			"attention", "sink_v", sink_v, [Some(config.n_heads), Some(config.head_dim)],
			&self.device, config.dtype
		)?;
		tensor_check::vector_shape(
			"attention", "attn_temperature", attn_temperature, config.n_heads,
			&self.device, DType::F32
		)?;
		tensor_check::matrix_shape(
			"attention", "out", out, [Some(seq_len), Some(config.o_stride)],
			&self.device, config.dtype
		)?;
		if let Some(l) = l {
			tensor_check::matrix_shape(
				"attention", "l", l, [Some(config.n_heads), Some(seq_len)],
				&self.device, DType::F32
			)?;
		}

		debug_assert!(self.metadata.Q_PER_BLOCK.is_power_of_two());
		if seq_len & (self.metadata.Q_PER_BLOCK - 1) != 0 {
			cold_path();
