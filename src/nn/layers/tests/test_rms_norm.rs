...
fn test_rms_norm() {
	...
	let out = rms_norm.forward(inp.clone(), &mut ctx).unwrap();
	...
	#[rustfmt::skip] let expected_d_inp = Tensor::literal_factory::<f32>(dev.clone()).new_2d(
		&[
	...
	let d_inp = rms_norm.backward(d_out.clone(), &mut ctx).unwrap();
