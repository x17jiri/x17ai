...
Usage:
python verify_attn.py --create-inputs          # generate q.bin, k.bin, v.bin
...
def load_attn_config():
	if not os.path.exists(ATTN_CONFIG_PATH):
		return DEFAULT_V_EQ_K, DEFAULT_WINDOW_SIZE
	try:
		with open(ATTN_CONFIG_PATH, "r", encoding="ascii") as config_file:
			config = json.load(config_file)
	except (OSError, ValueError, TypeError, json.JSONDecodeError):
		return DEFAULT_V_EQ_K, DEFAULT_WINDOW_SIZE
	return bool(config.get("V_EQ_K", DEFAULT_V_EQ_K)), int(config.get("WINDOW_SIZE", DEFAULT_WINDOW_SIZE))
...
def create_inputs(q_len, kv_len):
	...
	k_f64 = rng.standard_normal((kv_len, HEAD_CNT, QK_DIM))
	v_f64 = rng.standard_normal((kv_len, HEAD_CNT, V_DIM))
	...
	k_bf16 = torch.from_numpy(k_f64).to(torch.bfloat16)
	v_bf16 = torch.from_numpy(v_f64).to(torch.bfloat16)
	...
	flatten_heads(k_bf16).view(torch.int16).numpy().view(np.uint16).tofile("tmp/k.bin")
	flatten_heads(v_bf16).view(torch.int16).numpy().view(np.uint16).tofile("tmp/v.bin")
	...
	print(f"Created tmp/k.bin:  [{kv_len}, {HEAD_CNT * QK_DIM}] bf16  ({kv_len * HEAD_CNT * QK_DIM * 2} bytes)")
	print(f"Created tmp/v.bin:  [{kv_len}, {HEAD_CNT * V_DIM}] bf16  ({kv_len * HEAD_CNT * V_DIM * 2} bytes)")
...
def load_inputs(q_len, kv_len, large=False):
	...
	k_file = "tmp/large_k.bin" if large else "tmp/k.bin"
	v_file = "tmp/large_v.bin" if large else "tmp/v.bin"
	q_raw = np.fromfile(q_file, dtype=np.uint16).reshape(q_len, HEAD_CNT * QK_DIM)
	k_raw = np.fromfile(k_file, dtype=np.uint16).reshape(kv_len, HEAD_CNT * QK_DIM)
	Q = torch.from_numpy(q_raw.view(np.int16)).view(torch.bfloat16).reshape(q_len, HEAD_CNT, QK_DIM)
	K = torch.from_numpy(k_raw.view(np.int16)).view(torch.bfloat16).reshape(kv_len, HEAD_CNT, QK_DIM)
	if V_EQ_K:
		V = K[:, :, :V_DIM]
	else:
		v_raw = np.fromfile(v_file, dtype=np.uint16).reshape(kv_len, HEAD_CNT * V_DIM)
		V = torch.from_numpy(v_raw.view(np.int16)).view(torch.bfloat16).reshape(kv_len, HEAD_CNT, V_DIM)
	return Q, K, V
...
if __name__ == "__main__":
	...
	parser.add_argument("--create-inputs", action="store_true",
						help="Generate random tmp/q.bin, tmp/k.bin, and tmp/v.bin")
	parser.add_argument("--large", action="store_true",
						help="Use large generated data (tmp/large_q.bin, tmp/large_k.bin, tmp/large_v.bin)")
