...
constexpr usize OUT_N_PER_BLOCK = N_PER_BLOCK / 2;
...
// GEMM: C[A_ROWS, B_COLS/2] = GeGLU(A[A_ROWS, K] @ B[K, B_COLS])
...
__global__ void gemm_kernel(
	...
	usize out_cols = B_COLS / 2;
	bf16 *c_ptr = C + blockIdx.x * M_PER_BLOCK * out_cols + blockIdx.y * OUT_N_PER_BLOCK;
	GMatrix<bf16, M_PER_BLOCK, OUT_N_PER_BLOCK> gC_block{c_ptr, out_cols};
	X17_UNROLL for (usize mi = 0; mi < M_TILES; ++mi) {
		store(out[mi], gC_block, warp_m + mi * 16, (warp_n / 2));
	}
}
...
int main(int argc, char *argv[]) {
	...
	std::vector<bf16> h_A(A_ROWS * K), h_B(B_COLS * K), h_C(A_ROWS * (B_COLS / 2));
	...
	cudaMalloc(&d_C, A_ROWS * (B_COLS / 2) * sizeof(bf16));
	...
	printf("A=[%u, %u], B=[%u, %u] stored as B^T=[%u, %u], out=[%u, %u]\n", A_ROWS, K, K, B_COLS, B_COLS, K, A_ROWS, B_COLS / 2);
	printf("Average kernel time over %d runs: %.3f ms\n", NUM_RUNS, elapsed * 1e3);
	printf("%.2f TFLOPS\n", tflops);

	cudaMemcpy(h_C.data(), d_C, A_ROWS * (B_COLS / 2) * sizeof(bf16), cudaMemcpyDeviceToHost);
	...
	for (usize m = 0; m < 4; m++) {
		for (usize n = 0; n < 4; n++)
			printf(" %10.4f", float(h_C[m * (B_COLS / 2) + n]));
		printf("\n");
	}
