...
namespace Attn_kv_fwd {
	...
	using Writer = QKVGMatrixWriter<QK_DIM, VG_DIM, KV_PROJ_OUTPUTS, SPARSE_FAN_IN>;
...
int main(int argc, char *argv[]) {
	...
	if (!Kernel::has_full_output_tiles(seq_len, KV_PROJ_OUTPUTS)) {
		printf(
			"Expected KV output shape [%u, %u] to align with block shape [%u, %u]\n",
			seq_len,
			KV_PROJ_OUTPUTS,
			Kernel::M_PER_BLOCK,
			Kernel::N_PER_BLOCK
	...
	std::vector<b8::FixedI8> h_weights = load_i8_tensor(
		torch_tensor_path("attn_kv_weights_i8.bin"),
		KV_PROJ_OUTPUTS,
		SPARSE_FAN_IN
	);
	...
	dim3 grid = Kernel::output_grid(seq_len, KV_PROJ_OUTPUTS);
	...
	double tflops = 2.0 * KV_PROJ_OUTPUTS * SPARSE_FAN_IN * seq_len / (median_ms * 1e-3) / 1e12;
